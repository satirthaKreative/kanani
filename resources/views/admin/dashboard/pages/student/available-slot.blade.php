                                    ...
                                    <div class="ml-auto d-flex align-items-center">
                                        <nav>
                                            <ol class="breadcrumb p-0 m-b-0">
                                                ...
                                                <li class="breadcrumb-item active text-primary" aria-current="page">Available Slot Table</li>
                                            </ol>
                                        </nav>
                                    </div>
                                </div>
                                ...
                                <div class="card card-statistics">
                                    <div class="card-body">
                                        <div class="row mb-3">
                                            <div class="col-lg-12">
                                                <span>
                                                    <a class="btn btn-success text-white" onclick="add_available_slot_modal_fx()">Add Available Time Slot</a>
                                                </span>
                                            </div>
                                        </div>
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            ...
                                                            ?>
                                                            <span class="text-info text-bold"><a href="javascript:;" onclick="config_available_slot_edit_fx({{ $ck->id }})" class="btn btn-sm btn-info"><i class="fa fa-edit"></i></a></span> <span class="text-danger text-bold"><a href="javascript:;" class="btn btn-sm btn-danger" onclick="config_available_slot_del_fx({{ $ck->id }})"><i class="fa fa-trash"></i></a></span> <span class="text-info text-bold"><a href="javascript:;" class="btn btn-sm btn-success" onclick="config_available_slot_change_state_fx({{ $ck->id }},'{{ $update_state }}')">Change Status</a></span>
                                                        </td>
                                                    </tr>
                                                    @endforeach
                                                </tbody>
...
<div class="modal" id="add-available-slot-modal">
  <div class="modal-dialog">
    <div class="modal-content">

      <!-- Modal Header -->
      <div class="modal-header">
        <h4 class="modal-title">Add Available Slot</h4>
        <button type="button" class="close" data-dismiss="modal">&times;</button>
      </div>

      <!-- Modal body -->
      <div class="modal-body">
        <form action="{{ route('admin.student-available-slot-add') }}" method="POST">
            @csrf
...
<div class="modal" id="edit-available-slot-modal">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <!-- Modal body -->
      <div class="modal-body">
        <form action="{{ route('admin.student-available-slot-update') }}" method="POST">
            @csrf
...
<script>
    ...
    function add_available_slot_modal_fx(){
        $("#add-available-slot-modal").modal('show');
    }
    ...
    function config_available_slot_edit_fx(lang_val){
        $("#edit-available-slot-modal").modal('show');
        $.ajax({
            url: "{{ route('admin.student-available-slot-edit') }}",
            type: "GET",
            data: {id: lang_val},
            dataType: "json",
    ...
    function config_available_slot_del_fx(lang_val){
        var x = confirm('Are you sure to delete this?');
        if(x){
            $.ajax({
                url: "{{ route('admin.student-available-slot-delete') }}",
                type: "GET",
                data: {id: lang_val},
                dataType: "json",
                success: function(event){
    ...
    function config_available_slot_change_state_fx(lang_val,update_state){
        $.ajax({
            url: "{{ route('admin.student-available-slot-change-status') }}",
            type: "GET",
